$(document).ready(function () {
    ...
    if (currentSearchTerm.length >= 2) {
      ...
      $.ajax({
        ...
        error: function (xhr, status, error) {
          ...
          $("#searchdata").html(
            '<div class="text-danger">Error loading searched results</div>',
          );
        },
      });
    } else {
  ...
  $(document).on("click", ".AddToCartBtn", function () {
    ...
    var pqty = $(this)
      .closest(".product-detail-disc-wrapper")
      .find(".qty-input")
      .val();
    ...
    $.ajax({
      ...
      error: function (xhr, status, error) {
        console.error("AJAX Error: " + status + " - " + error);
        $("#cartfield").html(
          '<div class="text-danger">Error Adding product To The Cart</div>',
        );
      },
    });
  });
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $(".Get_SubTotal_Price").text(
        totalPrice.toLocaleString("en-PK", { minimumFractionDigits: 2 }) +
          " PKR",
      );
      if (totalPrice > 0) {
...
function loadCheckoutProduct() {
  $.ajax({
    ...
    error: function (xhr, status, error) {
      console.error("AJAX Error: " + status + " - " + error);
      $("#cart-item-area").html(
        '<div class="text-danger">Error loading cart items. Please try again.</div>',
      );
    },
  });
}
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      sessionStorage.setItem(
        `categoryProducts_${categoryid}`,
        data.product_Cards,
      );
      sessionStorage.setItem(
        `categoryTitle_${categoryid}`,
        data.category_title,
      );
      sessionStorage.setItem(`loadMoreHtml_${categoryid}`, data.load_more_html);
      let page = data.page;
      window.location.href =
        "view-category.php?categoryid=" + categoryid + "&page=" + page;
    },
    error: function () {
      sessionStorage.setItem(
        `categoryProducts_${categoryid}`,
